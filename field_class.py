...
class Field:
    """Simulate a field that can contain animals and crop"""
    ...
    def grow(self,light,food,water):
        #grow the crops (light and water available to all)
        if len(self._crops) > 0:
            for crop in self._crops:
                crop.grow(light,water)
        if len(self._animals) > 0:
            #grow the animals (water available to all animals but food
            #is a total that must be shared
            food_required = 0
            #get a total of the food required by the animals in the field
            for animal in self._animals:
                needs = animal.needs()
                food_required += needs["food need"]
            #if we have more food avialble than is required work out the additional
            #available food
            ...
            for animal in self._animals:
                ...
                if food >= needs["food need"]:
                    #remove food for this animal from the total
                    food -= needs["food need"]
                    feed = needs["food need"]
                    #see if there is any additional food for this animal
...
def select_crop(length_list):
    valid = False
    while not valid:
        selected = int(input("Please select an crop:"))
...
def display_animal_menu():
    ...
    print("2. Sheep")
    print()
    print("0. I dont want to add an animal - return me to the main menu")
    print()
    print("Please select an option from the above menu")
    
def display_main_menu():
    print()
    print("1. Plant an new crop")
    ...
    print("6. Grow field automaticallt over 30 days")
    print()
...
def add_animal_to_field(field):
    ...
    choice = get_menu_choice(0,2)
    if choice == 1:
        name = input("What do you want to call this cow: ")
        ...
        name = input("What do you want to call this sheep: ")
